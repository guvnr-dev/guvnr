...
describe('Slash Command Structure', () => {
  ...
  before(() => {
    commandFiles = readdirSync(COMMANDS_DIR).filter(f => f.endsWith('.md'));
  });
  ...
  it('should have output format specification', () => {
    for (const file of commandFiles) {
      ...
      assert.ok(hasOutputFormat(content), `${file} should have output format specification`);
    }
  });
});
...
describe('Plan Command', () => {
  ...
  it('should have required planning sections', () => {
    ...
    const requiredSections = ['Instructions', 'Output Format'];

    // Check for key planning concepts
    assert.ok(
      content.includes('understanding') || content.includes('Understanding'),
      'Should discuss understanding requirements'
    );
    assert.ok(
      content.includes('assumption') || content.includes('Assumption'),
      'Should discuss assumptions'
    );
    assert.ok(
      content.includes('approach') || content.includes('Approach'),
...
describe('Verify Command', () => {
  ...
  it('should have verification steps', () => {
    assert.ok(
      content.includes('Verification Steps') || content.includes('Steps'),
  ...
  it('should have verdict options', () => {
    assert.ok(
      content.includes('COMPLETE') || content.includes('✓'),
      'Should have complete verdict option'
    );
    assert.ok(
      content.includes('INCOMPLETE') || content.includes('✗'),
...
describe('Handoff Command', () => {
  ...
  it('should have session summary section', () => {
    assert.ok(
      content.includes('Summary') || content.includes('Accomplished'),
  ...
  it('should list next steps', () => {
    assert.ok(
      content.includes('Next') || content.includes('Recommended') || content.includes('step'),
  ...
  it('should mention file location for saving', () => {
    assert.ok(
      content.includes('docs/session-notes') || content.includes('session'),
...
describe('Review Command', () => {
  ...
  it('should have multiple perspectives', () => {
    ...
    assert.ok(
      perspectiveCount >= 3,
      `Should have at least 3 perspectives, found ${perspectiveCount}`
    );
  });
...
describe('Security Review Command', () => {
  ...
  it('should include statistics', () => {
    assert.ok(
      content.includes('%') || content.includes('86') || content.includes('88'),
      'Should include vulnerability statistics'
    );
  });

  it('should have verdict options', () => {
    assert.ok(content.includes('SECURE') || content.includes('✅'), 'Should have secure verdict');
    assert.ok(
      content.includes('INSECURE') || content.includes('🚫'),
...
describe('Refactor Command', () => {
  ...
  it('should have incremental approach', () => {
    assert.ok(
      content.includes('incremental') || content.includes('step') || content.includes('phase'),
...
describe('Test Coverage Command', () => {
  ...
  it('should analyze coverage', () => {
    assert.ok(
      content.includes('coverage') || content.includes('Coverage'),
  ...
  it('should identify gaps', () => {
    assert.ok(
      content.includes('gap') || content.includes('missing') || content.includes('untested'),
  ...
  it('should prioritize improvements', () => {
    assert.ok(
      content.includes('priorit') || content.includes('critical') || content.includes('high'),
...
describe('Subagent Structure', () => {
  ...
  it('should specify model in explorer agent', () => {
    ...
    assert.ok(
      explorerContent.includes('model:') || explorerContent.includes('haiku'),
