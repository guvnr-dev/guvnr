...
function validateClaudeMd(content) {
  const requiredSections = ["## Overview", "## Tech Stack", "## Current State"];
...
`,
    ...
    assert.ok(
      validation.valid,
      `CLAUDE.md should be valid. Missing: ${validation.requiredMissing.join(", ")}`,
    );
  });
  ...
  it("should create complete full preset structure", () => {
    ...
    // Validate MCP files
    assert.ok(
      existsSync(join(tempDir, "scripts/mcp/project-memory-server.py")),
      "MCP server should exist",
    );
    assert.ok(
      existsSync(join(tempDir, "scripts/metrics/collect-session-metrics.sh")),
...
`;
    ...
    assert.ok(
      validation.requiredMissing.includes("## Tech Stack"),
      "Should detect missing Tech Stack",
    );
    assert.ok(
      validation.requiredMissing.includes("## Current State"),
...
describe("Configuration Schema Validation", () => {
  ...
  it("should validate full preset config", () => {
    const fullConfig = {
      ...
      hooks: {
        enabled: true,
        scripts: ["post-edit", "verify-deps", "check-todos", "check-claude-md"],
      },
...
describe("Cross-Component Integration", () => {
  ...
  it("should have consistent command references across components", () => {
    ...
    for (const cmd of allCommands) {
      ...
      assert.ok(
        content.includes("---"),
        `Command ${cmd}.md should have frontmatter`,
      );
      assert.ok(
        content.includes("description:"),
        `Command ${cmd}.md should have description`,
      );
    }
  });
  ...
  it("should have consistent agent references across components", () => {
    ...
    for (const agent of allAgents) {
      ...
      assert.ok(
        content.includes("---"),
        `Agent ${agent}.md should have frontmatter`,
      );
      assert.ok(
        content.includes("name:"),
        `Agent ${agent}.md should have name`,
      );
    }
  });
  ...
  it("should have matching hook scripts", () => {
    ...
    for (const hook of allHooks) {
      ...
      assert.ok(
        ...
        `Hook ${hook} should have bash shebang`,
      );
    }
  });
  ...
  it("should have all preset templates", () => {
    ...
    for (const preset of presets) {
      ...
      assert.ok(
        ...
        `Preset directory ${preset} should exist`,
      );
      ...
      assert.ok(
        ...
        `Preset ${preset} should have config.json`,
      );
      assert.ok(
...
console.log(
  "Integration tests loaded. Run with: node --test tests/integration.test.js",
);
